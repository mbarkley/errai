...
Errai allows you to do this by setting the [code]+onKeyUp+ flag in the [code]+@Bound+ annotation. *_Note that this parameter can only be used on Widgets that extend GWT's ValueBoxBase widget_*. Setting this parameter to [code]+true+ will cause the model value to update on a KeyUpEvent as well as the default ValueChangeEvent. This will result in the model object being updated as soon as any text is entered/removed from the Widget. You can specify this as follows:
...
[source, ini]
----
# The following blacklists the class some.fully.qualified.ClassName and all classes
...
you should always [code]+@Inject+ references to your Composite components.
...
As previously mentioned, components in Errai UI consist of a Java class (the _templated bean_), an HTML file (the _template_),
and an optional CSS file (the _template stylesheet_). A Java class is a templated bean if the type is annotated with +@Templated+.
...
[NOTE]
.Uses DOM wrappers from Errai Common
====
The +@DataField+ types used in this example are thin HTML wrappers found in the +org.jboss.errai.common.client.dom+ package of errai-common.
It is still possible to use widgets in a +@DataField+ but this approach is not recommended.
====
...
The previous [code]+LoginForm+ example uses HTML wrapper elements from Errai Common, but there are other types that can be used as [code]+@DataFields+.
Here is a full list:

. Any native [code]+@JsType+ that is a wrapper for a DOM element (including those provided in errai-common).

. Any Errai UI component.

. Any [code]+com.google.gwt.dom.client.Element+ type.

. Any type implementing [code]+org.jboss.errai.common.client.api.IsElement+.
...
@Templated
public class MyBean {
  @Inject @DataField Div root;
}
----

This method has the advantage that any valid @DataField type can be used.

===== Implement org.jboss.errai.ui.client.local.api.IsElement

This interface provides a default implementation of [code]+org.jboss.errai.common.client.api.IsElement+ that works for any templated bean. For example:
...
But consider the heading tags (h1, h2, ..., h6). In gwt-user and errai-common these are all represented by [code]+HeadingElement+.
...
. Element attributes are merged from the template to the templated bean +@DataFields+
...
When styling your templates, you should keep in mind that all attributes defined in the template file will take precedence over any preset attributes in your [code]+@DataFields+.
...
If styles must be applied in Java instead of in the template, [code]+@PostConstruct+ or other methods should be favored over constructors to apply styles to fully-constructed components.

==== DOM Elements (templated bean field wins)

Element composition, however, functions inversely from attribute merging, and the [code]+<span>+ defined in our template is actually replaced by the [code]+<div>+
because of the Div in our component field. This does not, however, change the behavior of the attribute merge - the new [code]+<div>+ was still be rendered inline,
...
Additionally, because [code]+Div+ is a thin Element wrapper the contents of the <span> "field-1" Element in the template were preserved;
...
[WARNING]
.User cookie is deprecated
====
For users with many roles, the serialized string may be too large to fit in a cookie. For this reason, the user cookie is not recommended and is kept only for backwards compatibility.
====
...
** org.hibernate.javax.persistence:hibernate-jpa-2.1-api:jar:1.0.1.Final:compile
** org.jboss.errai:errai-ioc:jar:compile
*** org.jboss.errai:errai-config:jar:compile
*** org.jboss.errai:errai-codegen:jar:compile
**** org.jboss.errai:errai-common:jar:compile
...
** org.hibernate.javax.persistence:hibernate-jpa-2.1-api:jar:1.0.1.Final:provided
...
* Remove any usage of the `@StyleDescriptor`. Plain CSS files can be declared in Java code via the `@Templated` annotation. Errai no longer provides annotations for using LESS.
