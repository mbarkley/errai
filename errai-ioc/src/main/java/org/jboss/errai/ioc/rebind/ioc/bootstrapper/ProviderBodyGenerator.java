...
public class ProviderBodyGenerator extends AbstractBodyGenerator {
  ...
  private List<Statement> getAndInvokeProvider(final Injectable injectable, final Dependency providerDep) {
    ...
    statement
            .add(castTo(
                    parameterizedAs(Injector.class,
                            typeParametersOf(
                                    parameterizedAs(Provider.class, typeParametersOf(injectable.getInjectedType())))),
                    loadVariable("contextManager")
                            .invoke("getInstance", providerInjectable.getInjectorClassSimpleName()).invoke("get")
                            .returnValue()));
